...
Private Const AIM_MD5_STRING = "AOL Instant Messenger (SM)"
...
Public Function MD5StringChange(lngnum As Long) As String
        ...
        bytA = lngnum And &HFF&
        MD5StringChange = Chr(bytA)
        bytB = (lngnum And &HFF00&) \ 256
        MD5StringChange = MD5StringChange & Chr(bytB)
        bytC = (lngnum And &HFF0000) \ 65536
        MD5StringChange = MD5StringChange & Chr(bytC)
        If lngnum < 0 Then
            bytD = ((lngnum And &H7F000000) \ 16777216) Or &H80&
        Else
            bytD = (lngnum And &HFF000000) \ 16777216
        End If
        MD5StringChange = MD5StringChange & Chr(bytD)
End Function

Public Function MD5Value() As String

    MD5Value = MD5StringChange(arrLongConversion(1)) & MD5StringChange(arrLongConversion(2)) & MD5StringChange(arrLongConversion(3)) & MD5StringChange(arrLongConversion(4))

End Function

Public Function AIMEncryptPw(strChallange As String, strPassword As String) As String
    AIMEncryptPw = Encrypt(strChallange & Encrypt(strPassword) & AIM_MD5_STRING)
End Function
...
Public Function Encrypt(strData As String) As String
    ...
    bytBuffer = MD5StringArray(strData)
    
    MD5Start
    MD564Split Len(strData), bytBuffer
    MD5Finish
