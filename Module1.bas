...
Function GetLength(ByteLength As String) As Long
If Len(ByteLength) >= 3 Then
    GetLength = (Asc(Mid(ByteLength, 1, 1)) * 256 * 256) + (Asc(Mid(ByteLength, 2, 1)) * 256) + (Asc(Mid(ByteLength, 3, 1)) * 1)
End If
End Function

'Give this a three byte value (four byte but first byte is assumed a value of 0) and it returns a numeric value
Function MessyLength(ByteLength As String) As Long
MessyLength = 0
For i = 1 To Asc(Mid(ByteLength, 1, 1))
    MessyLength = MessyLength + 16777216
    DoEvents
Next i
For i = 1 To Asc(Mid(ByteLength, 2, 1))
    MessyLength = MessyLength + 65536
    DoEvents
Next i
For i = 1 To Asc(Mid(ByteLength, 3, 1))
    MessyLength = MessyLength + 256
    DoEvents
Next i
MessyLength = MessyLength + Asc(Mid(ByteLength, 4, 1))
End Function
...
Function OBL(strPoop)
    OBL = Chr(Len(strPoop)) & strPoop
End Function
...
Function ChrH(strPoop) As String
ChrH = ChrA("H" & Replace(strPoop, " ", " H"))
End Function
...
Function removeHTML(TheStuff)
Dim U1
removeHTML = ""
For i = 1 To Len(TheStuff)
    If Mid(TheStuff, i, 1) = "<" Then
        For X = 1 To Len(TheStuff) - i
            If Mid(TheStuff, i + X, 1) = ">" Then
                i = i + X
                GoTo Skip:
            End If
            DoEvents
        Next X
    End If
    removeHTML = removeHTML & Mid(TheStuff, i, 1)
Skip:
    DoEvents
Next i
End Function

Function ErinDauphine(strMessage As String) As String
Dim U1, isCapped As Boolean, isHTML As Boolean
ErinDauphine = ""
isCapped = False
isHTML = False
For i = 1 To Len(strMessage)
    If Mid(strMessage, i, 1) = "<" Then
        isHTML = True
    ElseIf Mid(strMessage, i, 1) = ">" Then
        isHTML = False
    End If
    If isHTML = False Then
        If isCapped Then
            ErinDauphine = ErinDauphine & LCase(Mid(strMessage, i, 1))
            isCapped = False
        Else
            ErinDauphine = ErinDauphine & UCase(Mid(strMessage, i, 1))
            isCapped = True
        End If
    Else
        ErinDauphine = ErinDauphine & Mid(strMessage, i, 1)
    End If
    DoEvents
Next i
End Function
...
Public Sub RemoveDupes(lst As ListBox, Optional boolHide As Boolean = False)
    Dim iPos As Integer, ASSy
    iPos = 0
    '-- if listbox empty then exit..
    If lst.ListCount < 1 Then Exit Sub

    If boolHide = True Then lst.Visible = False
    ...
    '     ine..
    lst.ListIndex = 0
    If boolHide = True Then lst.Visible = True
End Sub
...
Public Function StringToIP(strStrin) As String
    On Error Resume Next
    StringToIP = ""
    For i = 1 To Len(strStrin)
        StringToIP = StringToIP & Asc(Mid(strStrin, i, 1)) & "."
        DoEvents
    Next i
    If StringToIP = "" Then
        StringToIP = Chr(0)
        Exit Function
    End If
    StringToIP = Left(StringToIP, Len(StringToIP) - 1)
End Function

Public Function TrimNum(strStrin) As String
    TrimNum = ""
    For i = 1 To Len(strStrin)
        If Not IsNumeric(Mid(strStrin, i, 1)) Then
            TrimNum = TrimNum & Mid(strStrin, i, 1)
        End If
        DoEvents
    Next i
End Function

Public Function BinaryPos(intNumber As Integer) As String

    Dim Dennum As Single 'Denary Number
    Dim Remainder(15) As Single '16 bits
    Dim i, j As Integer 'Counter
    
    BinaryPos = ""
    i = 0
    
    Dennum = Val(intNumber)

    For i = 0 To 15
        Remainder(i) = Dennum Mod 2
        Dennum = Dennum / 2
        Dennum = Int(Dennum)
    Next i
    
    For i = 0 To 15
        BinaryPos = BinaryPos & Remainder(15 - i)
    Next i
    
End Function

Public Function MyTruePath() As String
    
    Dim tempAss
    tempAss = App.Path
    If Right(tempAss, 1) <> "\" Then
        MyTruePath = tempAss & "\"
    Else
        MyTruePath = tempAss
    End If
